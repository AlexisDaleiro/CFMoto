...
Phase 0–8 están documentadas e implementadas. Compare admite hasta tres modelos de la misma categoría, persiste solo slugs y deriva su matriz exclusivamente de especificaciones verificadas. Locations públicas son `needsClientValidation`: solo Service está afirmado y Dealer/WhatsApp/horarios/coordenadas no se infieren. Quote y Test Ride son demo frontend sin transmisión ni persistencia de PII. `experiences` gobierna Racing, Adventure, Tecnología y Comunidad; el copy nuevo es `proposal`, los hechos de producto se referencian desde datos verificados y la falta de media se resuelve con composición gráfica, nunca con relatos o claims ficticios. La dirección oficial es **Hybrid — Apex + Engineered Clarity** dentro de un único Design System, nunca dos temas o sistemas paralelos. Principio rector: **Expressive where emotion matters. Precise where decisions matter.** Los assets `src/assets/prototypes/450sr-concept-*` son `proposal`, no oficiales, y deben reemplazarse antes de producción. Cyan `#00B7E5` y Barlow Condensed + Inter siguen provisionales. El próximo alcance requiere brief explícito.
